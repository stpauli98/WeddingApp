...
const AdminDashboardTabs: React.FC<AdminDashboardTabsProps> = ({ guests, event }) => {
  ...
  // QR COLOR STATE
  const defaultQrColor = "var(--primary)"; // Koristi primarnu boju iz trenutne teme
  ...
  <section className="w-full max-w-2xl mx-auto mb-8">
    <div className="flex flex-col md:flex-row items-center justify-center gap-8 p-6 bg-primary/5 rounded-2xl shadow-lg border border-primary/20">
      {/* QR Vizuelni blok */}
      <div ref={qrRef} className="flex flex-col items-center justify-center bg-card rounded-xl shadow-md p-4 border border-muted">
        <QRCodeCanvas value={guestUrl} size={148} bgColor="var(--card)" fgColor={qrColor} includeMargin={true} className="rounded-xl" />
        <label className="flex items-center gap-2 mt-2 mb-1 text-xs text-muted-foreground cursor-pointer" htmlFor="qrColorPicker">
        ...
        <button
          ...
          className="mt-2 flex items-center gap-2 px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-semibold shadow transition"
          title="Preuzmi QR kod"
        >
          <Download className="w-5 h-5" /> Preuzmi QR kod
        </button>
        <span className="mt-2 text-xs text-muted-foreground text-center max-w-[180px]">Skenirajte QR kod za direktan pristup stranici za goste.</span>
      </div>
      {/* Link + Akcije */}
      <div className="flex flex-col items-center md:items-start gap-3 w-full max-w-xs">
        <span className="font-bold text-foreground text-base mb-1">Link za goste</span>
        <div className="flex items-center gap-2 w-full">
          <span className="px-3 py-2 rounded-lg bg-card border border-muted text-xs font-mono select-all break-all max-w-[220px] md:max-w-xs truncate shadow-sm" title={guestUrl}>{guestUrl}</span>
          <button
            onClick={handleCopy}
            className={`flex items-center gap-1 px-3 py-2 rounded-lg ${copied ? 'bg-accent text-accent-foreground' : 'bg-primary hover:bg-primary/90 text-primary-foreground'} text-xs font-semibold shadow transition`}
            title="Kopiraj link"
          >
            {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />} {copied ? 'Kopirano!' : 'Kopiraj'}
          </button>
        </div>
        <span className="text-xs text-muted-foreground mt-1">Podelite ovaj link sa gostima ili ga pošaljite putem pozivnice.</span>
        <div className="mt-3 w-full bg-primary/10 rounded-md p-3 text-xs text-foreground flex items-center gap-2 shadow-inner">
          <svg className="w-5 h-5 text-primary shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 8v.01" /></svg>
      ...
      <TabsList
        className="overflow-x-auto whitespace-nowrap scrollbar-hide flex gap-1 md:gap-2 px-1 sticky top-0 bg-background/90 z-20"
        ...
        <>
          {guests.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
              ...
              <div className="text-sm text-muted-foreground">Kada se gosti registruju za ovaj event, ovde će se pojaviti njihovi profili i slike.</div>
            </div>
          ) : (
