  ...
  if (!images.length) {
    return (
      <motion.div 
        ...
        className="flex flex-col items-center justify-center py-16 text-[hsl(var(--lp-muted-foreground))] rounded-lg bg-[hsl(var(--lp-muted))]/20 border border-[hsl(var(--lp-accent))]/20 shadow-sm"
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="w-20 h-20 mb-6 opacity-40 text-[hsl(var(--lp-accent))]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {isLoading && (
        <div className="w-full flex justify-center items-center py-20">
          <div className="animate-pulse flex flex-col items-center">
            <div className="w-16 h-16 rounded-full bg-[hsl(var(--lp-accent))]/20 flex items-center justify-center mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-[hsl(var(--lp-accent))]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {sortedImages.map((img, idx) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 w-full p-3 z-20">
              {img.guestName && (
                <div className="text-[hsl(var(--lp-text))] text-sm font-medium truncate bg-[hsl(var(--lp-muted))]/40 px-3 py-2 rounded-full backdrop-blur-sm">
                  {img.guestName}
                </div>
              )}
            </div>
            <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
              <span className="bg-[hsl(var(--lp-accent))] text-[hsl(var(--lp-text))] text-xs font-medium px-2.5 py-1 rounded-full">
          ...
          >
            ...
            >
              ...
              <motion.div
                ...
                className="mt-4 text-[hsl(var(--lp-text))] text-base bg-[hsl(var(--lp-muted))]/60 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg flex items-center gap-2"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-[hsl(var(--lp-accent))]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span className="text-[hsl(var(--lp-primary-foreground))] text-sm font-medium">{sortedImages[currentIdx].guestName || "Nepoznat gost"}</span>
                <span className="text-xs text-[hsl(var(--lp-muted-foreground))] ml-2">
              ...
              <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-1">
                {sortedImages.length > 1 && (
                  <div className="flex items-center justify-center gap-1 bg-[hsl(var(--lp-muted))]/40 backdrop-blur-sm rounded-full px-3 py-1.5">
                    ...
                    <span className="text-xs text-[hsl(var(--lp-text))] font-medium ml-2">{currentIdx + 1}/{sortedImages.length}</span>
                  </div>
                )}
              </div>
            </motion.div>
